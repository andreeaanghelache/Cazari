...
$.widget("bootstrap.bwizard", {
	...
	_createButtons: function () {
		...
		if (!this.buttons) {
			...
			if(nextBtnText != ''){
				this.nextBtn =
					$("<li class='next'><a class='btn ' href='#'>" +
						nextBtnText + "</a>")
					.appendTo(this.buttons).bind({
						'click': function () {
							self.next();
	...
	_pageLize: function (init) {
		...
		if (init) {
			...
			this.element.addClass('well bwizard clearfix');
			if (this.list) {
				this.list
			...
			this.container = $('<div/>');
			this.container.addClass('wizard-main');
